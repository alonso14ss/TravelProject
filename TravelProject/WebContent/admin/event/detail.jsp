...
<div class="container">
  <form>
  	...
  	<select name="product_id" id="eventSelect">
			...
			<option <%if(product.getProduct_id() == event.getProduct().getProduct_id()){ %>selected<%} %> value="<%=product.getProduct_id()%>"><%=product.getProduct_name() %></option>
			<%}%>
		</select>  
    <input type="text" name="discount" value="<%=event.getDiscount()%>">
    <div class="single-tab-select-box">
     <h2>이벤트 시작 시간</h2>
       <div class="travel-check-icon">
      <input type="text" id="departuredate" name="starttimeOri" class="form-control"
         data-toggle="datepicker" value="<%=event.getStarttimeOri()%>">
                                       </div>
                                       <!-- /.travel-check-icon -->
                                       <select name="start_time">
										<%for(int i=0;i<24;i++){%>
										<option <%if(event.getStart_time() ==i){ %>selected<%} %> value="<%=i%>"><%=i+"시"%></option>
											<%}%>
										</select> 
                                    </div>
     ...
     <h2>이벤트 종료 시간</h2>
      ...
      <input type="text" name="endtimeOri" class="form-control"
         data-toggle="datepicker" value="<%=event.getEndtimeOri()%>">
                                       </div>
                                       <!-- /.travel-check-icon -->
                                       <select name="end_time">
										<%for(int i=0;i<24;i++){%>
										<option <%if(event.getEnd_time() ==i){ %>selected<%} %> value="<%=i%>"><%=i+"시"%></option>
											<%}%>
										</select> 
                                    </div>
    ...
    <p>
  		...
  		<button type="button" onClick="edit()">이벤트 상품 수정</button>
  		<button type="button" onClick="del()">이벤트 상품 삭제</button>
